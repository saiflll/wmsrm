...
const IMPORT_CONFIGS: ImportConfig[] = [
    ...
    {
        key: 'planning-outbound',
        title: 'Planning Outbound',
        description: 'Import bulk rencana planning outbound (reservasi stok).',
        color: 'orange',
        icon: <IconClipboardList size={20} />,
        filename: 'Template_Planning_Outbound.xlsx',
...
const fetchRefData = async (apiInstance: any): Promise<RefData> => {
    ...
    return {
        ...
        stocks: (() => { const d = unwrap(stockRes); return Array.isArray(d) ? d : d?.data || []; })(),
...
export default function ImportPage() {
    ...
    const downloadTemplateGroup = (keys: string[], filename: string) => {
        const wb = XLSX.utils.book_new();
        IMPORT_CONFIGS.filter(c => keys.includes(c.key)).forEach((config) => {
            const ws = XLSX.utils.aoa_to_sheet([config.headers, ...config.sampleRows]);
            XLSX.utils.book_append_sheet(wb, ws, config.key);
        });
        ...
        <Box>
            <Box style={{ background: '#fff', borderLeft: '4px solid #12b886', padding: '14px 20px', marginBottom: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                <Group justify="space-between" align="center">
                    <Title order={4} style={{ color: '#111827', fontWeight: 800, display: "flex", alignItems: "center", gap: 8 }}>
                        <IconUpload size={20} style={{ color: '#12b886' }} />
                        IMPORT DATA WMS
                    </Title>
                    <Button
                        size="xs"
            ...
            <Paper withBorder p="md" mb="md" radius="md" style={{ background: '#fff' }}>
                <FileInput
                    ...
                    description="Pilih file Excel yang ingin diimport. Sheet akan otomatis dikenali dan diproses."
                ...
                <Accordion variant="separated" defaultValue="inventory">
                    <Accordion.Item value="inventory">
                        <Accordion.Control icon={<IconBoxSeam size={20} color="var(--mantine-color-blue-filled)" />}>
                            <Text fw={700}>Inventory & Transaksi</Text>
                            <Text size="xs" c="dimmed">Inbound, Outbound, Opname</Text>
                        </Accordion.Control>
                        <Accordion.Panel>
                            <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
                                {IMPORT_CONFIGS.filter(c => ['inbound', 'outbound', 'opname'].includes(c.key)).map(config => (
                                    <Paper key={config.key} withBorder p="sm" radius="md">
                                        <Group gap="sm" mb="xs">
                                            <Box style={{ color: `var(--mantine-color-${config.color}-filled)` }}>{config.icon}</Box>
                                            <Text fw={600} size="sm">{config.title}</Text>
                                        </Group>
                                        <Button size="xs" variant="light" fullWidth leftSection={<IconDownload size={14} />} onClick={() => downloadTemplate(config)}>
                                            Template {config.key}
                                        </Button>
                                    </Paper>
                                ))}
                            </SimpleGrid>
                            <Button mt="md" variant="light" color="blue" fullWidth leftSection={<IconDownload size={14} />} onClick={() => downloadTemplateGroup(['inbound', 'outbound', 'opname'], 'Template_Inventory.xlsx')}>
                                Download Gabungan Template Inventory
                            </Button>
                        </Accordion.Panel>
                    </Accordion.Item>

                    <Accordion.Item value="master">
                        <Accordion.Control icon={<IconDatabase size={20} color="var(--mantine-color-teal-filled)" />}>
                            <Text fw={700}>Master Data</Text>
                            <Text size="xs" c="dimmed">Produk, Lokasi Rak, Customer / Supplier</Text>
                        </Accordion.Control>
                        <Accordion.Panel>
                            <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
                                {IMPORT_CONFIGS.filter(c => ['produk', 'lokasi', 'customer'].includes(c.key)).map(config => (
                                    <Paper key={config.key} withBorder p="sm" radius="md">
                                        <Group gap="sm" mb="xs">
                                            <Box style={{ color: `var(--mantine-color-${config.color}-filled)` }}>{config.icon}</Box>
                                            <Text fw={600} size="sm">{config.title}</Text>
                            ...
                            <Button mt="md" variant="light" color="teal" fullWidth leftSection={<IconDownload size={14} />} onClick={() => downloadTemplateGroup(['produk', 'lokasi', 'customer'], 'Template_Master_Data.xlsx')}>
                                Download Gabungan Template Master
                            </Button>
                        </Accordion.Panel>
                    </Accordion.Item>

                    <Accordion.Item value="admin">
                        <Accordion.Control icon={<IconUsers size={20} color="var(--mantine-color-gray-filled)" />}>
                            <Text fw={700}>Lainnya / Lanjutan</Text>
                            <Text size="xs" c="dimmed">Users, Picking Plan, Relocation, Inbound Planning</Text>
                        </Accordion.Control>
                        <Accordion.Panel>
                            <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing="md">
                                {IMPORT_CONFIGS.filter(c => ['users', 'planning-outbound', 'relocation', 'inbound-planning'].includes(c.key)).map(config => (
                                    <Paper key={config.key} withBorder p="sm" radius="md">
                                        <Group gap="sm" mb="xs">
                                            <Box style={{ color: `var(--mantine-color-${config.color}-filled)` }}>{config.icon}</Box>
                                            <Text fw={600} size="sm">{config.title}</Text>
