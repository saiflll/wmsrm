...
export class PlanningOutboundService {
  constructor(
    @InjectRepository(PlanningOutbound) private repo: Repository<PlanningOutbound>,
  ...
  async findWithFilter(filter: { status?: string; dateFrom?: string; dateTo?: string }) {
    const where: any = {};
    if (filter.status) where.status = filter.status;
    if (filter.dateFrom && filter.dateTo) {
      where.created_at = Between(new Date(filter.dateFrom), new Date(filter.dateTo + 'T23:59:59'));
    }
  ...
  async update(id: number, dto: UpdatePlanningOutboundDto) {
    ...
    if (dto.customer_id !== undefined) {
      item.customer = (dto.customer_id
        ? await this.customerRepo.findOneBy({ id: dto.customer_id })
        : null) as any;
    }
    if (dto.shift_id !== undefined) {
      item.shift = (dto.shift_id
        ? await this.shiftRepo.findOneBy({ id: dto.shift_id })
        : null) as any;
    }
    if (dto.tanggal_planning) item.tanggal_planning = new Date(dto.tanggal_planning);
