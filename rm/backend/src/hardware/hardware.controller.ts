...
export class HardwareController {
    ...
    async sniff(@Body() payload: HwPayload) {
        ...
        if (payload.sku || payload.barcode) {
            barang = await this.barangRepo.findOneBy({ sku: payload.sku || payload.barcode });
        }
        ...
        return {
            ...
            matched_product: barang ? { id: barang.id, nama: barang.nama, sku: barang.sku } : null,
            matched_location: gudang ? { id: gudang.id, name: gudang.name, zone: (gudang as any).zone } : null,
            raw: payload,
